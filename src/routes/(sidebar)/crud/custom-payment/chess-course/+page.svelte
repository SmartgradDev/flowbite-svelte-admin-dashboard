<script lang="ts">
	...
	import { CUSTOM_PAYMENT_TYPE_2 } from '../../../../../utils/global_variable';

	const apiUrl = process.env.VITE_API_URL;
	
	// Set payment_type to chess-course for this specific route
	const payment_type = CUSTOM_PAYMENT_TYPE_2;
	
	console.log('Chess Course Admin - Payment Type:', payment_type); // Debug log
	...
	// Send Chess course emails
	const sendChessCourseEmails = async () => {
		...
		try {
			...
			const response = await axios.post(
				`${apiUrl}/admin/sendChessCourseEmails`,
...
<MetaTag
	title="Custom Payment - Chess Course"
	description="Manage custom payments for Chess Course"
/>
...
<main class="relative h-full w-full overflow-y-auto bg-gray-50 p-4 dark:bg-gray-900">
	<div class="mx-auto max-w-screen-2xl">
		<Breadcrumb class="mb-5">
			...
			<BreadcrumbItem>Custom Payment - Chess Course</BreadcrumbItem>
		</Breadcrumb>
		<Heading tag="h1" class="text-xl font-semibold text-gray-900 dark:text-white sm:text-2xl">
			Custom Payment - Chess Course
		</Heading>
		<p class="mb-6 text-gray-500 dark:text-gray-400">
			Manage custom payments for Chess Course
		</p>
		...
		<div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
			<Toolbar embedded class="w-full py-4 text-left text-gray-500 dark:text-gray-400">
				<ToolbarGroup>
					...
					<Button 
						on:click={sendChessCourseEmails} 
					...
					>
						...
						{#if emailSending}
							Sending Emails...
						{:else}
							Send Chess Course Emails ({selectedPayments.length})
						{/if}
					</Button>
				</ToolbarGroup>
				<ToolbarGroup>
					...
					<div class="flex">
						...
						<div class="ml-3">
							<p class="text-sm text-blue-700 dark:text-blue-200">
								...
								Click "Send Chess Course Emails" to notify these users about their verified registration.
							</p>
						</div>
					</div>
				</div>
