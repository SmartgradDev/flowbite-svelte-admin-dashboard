<script lang="ts">
	...
	import { CUSTOM_PAYMENT_TYPE_3 } from '../../../../../utils/global_variable';

	const apiUrl = process.env.VITE_API_URL;
	
	// Set payment_type to SummerCamp for this specific route
	const payment_type = CUSTOM_PAYMENT_TYPE_3;
	
	console.log('Summer Camp Admin - Payment Type:', payment_type); // Debug log
	...
	// Send Summer Camp emails
	const sendSummerCampEmails = async () => {
		...
		try {
			...
			const response = await axios.post(
				`${apiUrl}/admin/sendSummerCampEmails`,
...
<MetaTag
	title="Custom Payment - Summer Camp"
	description="Manage custom payments for Summer Camp"
/>
...
<main class="relative h-full w-full overflow-y-auto bg-gray-50 p-4 dark:bg-gray-900">
	<div class="mx-auto max-w-screen-2xl">
		<Breadcrumb class="mb-5">
			...
			<BreadcrumbItem>Custom Payment - Summer Camp</BreadcrumbItem>
		</Breadcrumb>
		<Heading tag="h1" class="text-xl font-semibold text-gray-900 dark:text-white sm:text-2xl">
			Custom Payment - Summer Camp
		</Heading>
		<p class="mb-6 text-gray-500 dark:text-gray-400">
			Manage custom payments for Summer Camp 2026
		</p>
				...
				<ToolbarGroup>
					...
					>
						...
						{#if emailSending}
							Sending Emails...
						{:else}
							Send Summer Camp Emails ({selectedPayments.length})
						{/if}
					</Button>
				</ToolbarGroup>
				<ToolbarGroup>
					...
					<div class="flex">
						...
						<div class="ml-3">
							<p class="text-sm text-blue-700 dark:text-blue-200">
								...
								Click "Send Summer Camp Emails" to notify these users about their verified registration.
							</p>
						</div>
					</div>
				</div>
